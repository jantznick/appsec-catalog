...
- **Whether secrets detection exists at all.** There's no field for it. It's a required control carrying the only hard, tier-independent block in the baseline, and nothing on your record can say you do it. Keep it in your pipeline config and tell AppSec. First in line on the roadmap.
...
- **Container, IaC, and SBOM coverage.** No fields. Note that Orbit's dependency inventory is derived from your repository's manifest files, which is a different artifact from an SBOM generated at build time from what actually went into the image — don't treat the Dependencies page as satisfying 3.9.
