...
- **Whether secrets detection exists at all.** There's no field for it yet. It's a required control carrying the only hard, tier-independent block in the baseline, and nothing on your record can say you do it. Keep it in your pipeline config and tell AppSec. Fields are coming on the same pattern as the other categories — tool, integration level, last scan date — and for GitHub-linked repositories, secret scanning and push protection are readable through the API, so this one ends up confirmed rather than self-reported.
