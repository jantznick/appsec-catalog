...
- **Whether the deploy path is actually segregated.** Only half of this is confirmable. The code-review half comes from your repository's branch protection — see [Build & Commit](/docs/phase-build-commit). The deploy half, that production is reachable only through the pipeline, needs Orbit to model environments, which it doesn't yet. Until it does, that half is something you assert.
