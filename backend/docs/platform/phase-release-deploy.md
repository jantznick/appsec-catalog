...
- **Whether the deploy path is actually segregated.** The code-review half of segregation of duties is evidenced from your repository's branch protection — see [Build & Commit](/docs/phase-build-commit). The deploy half, that production is reachable only through the pipeline, depends on Orbit modelling environments, which it doesn't do yet. Until it does, that half is something you assert rather than something anyone can confirm.
