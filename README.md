...
- **User Accounts**: Login via corporate Okta single sign-on (OIDC) or, for existing accounts, password/magic code. New users are provisioned through Okta; self-service password sign-up is disabled. Existing password users can switch to Okta seamlessly (matched by email). Magic codes can be emailed but for now they're just printed out to the console. See [SETUP.md](./SETUP.md#okta-sso-single-sign-on) for Okta configuration.
